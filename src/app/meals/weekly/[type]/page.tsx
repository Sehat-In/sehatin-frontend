...
const WeeklyMealPlanPage = ({ params }: { params: { type: string } }) => {
  const router = useRouter();
  const { type } = params;
  const [weeklyMeals, setWeeklyMeals] = useState<WeeklyMealPlan | null>(null);
    ...
    const fetchMeals = async () => {
      try {
        const response = await fetch(
          `https://meal-service.sehat-in.com/api/v1/meals/weekly-meal-plan?type=${type}`
        );
        if (!response.ok) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <h1 className="text-5xl font-bold text-center mb-12 text-gray-800">
        Weekly {type.charAt(0).toUpperCase() + type.slice(1)} Meal Plan
      </h1>
        ...
        {daysOfWeek.map((day) => (
          ...
          >
            <details className="group">
              <summary className="p-6 bg-gray-200 cursor-pointer font-semibold text-xl">
                {day}
              </summary>
