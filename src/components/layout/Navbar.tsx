    ...
    axios
      .get(
        process.env.NEXT_PUBLIC_API_URL +
          "/api/v1/posts/check-notification/" +
          userData.username
      )
      .then((res) => {
      ...
      <div className="navbar bg-base-100" style={{ backgroundColor: "teal" }}>
        ...
        <div className="flex-none" style={{ gap: 10 }}>
          {!isAuthenticated ? (
            ...
            <Button onClick={handleGoToProgress}>Progress</Button>
          )}
          {!isAuthenticated ? (
            <></>
          ) : (
            <Button onClick={handleGoToForum}>Forum</Button>
          )}
          {!isAuthenticated ? (
            <></>
          ) : (
            <Button onClick={handleGoToMeals}>Meals</Button>
          )}
          {!isAuthenticated ? (
            <></>
          ) : (
            <Button onClick={handleGoToWorkouts}>Workouts</Button>
          )}
          {!isAuthenticated ? (
            <></>
          ) : notification ? (
